...
class BiLSTM(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_size: int, hidden_size: int, n_layers: int, n_outputs: int, device: torch.device) -> None:
        ...
        Args:
            ...
            device: the device is being used
        """
        super().__init__()
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        """
        h0 = (torch.zeros(self.n_layers * 2, X.size(0), self.hidden_size).to(self.device), torch.zeros(self.n_layers * 2, X.size(0), self.hidden_size).to(self.device))
        out = self.embed(X)
